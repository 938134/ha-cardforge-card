...
import { unsafeHTML } from 'https://unkg.com/lit-html/directives/unsafe-html.js?module';
import { BlockRegistry } from './blocks/block-registry.js';
import { designSystem } from './core/design-system.js';

// 检查是否已经定义过自定义元素
const CARD_ELEMENT_NAME = 'ha-cardforge-card';
...
if (!customElements.get(CARD_ELEMENT_NAME)) {
      ...
      return html`
        <ha-card>
          <div class="loading-container">
            ...
            <div class="cf-text-md cf-mt-sm">初始化中...</div>
          </div>
        </ha-card>
      `;
    }
  ...
  customElements.define(CARD_ELEMENT_NAME, HaCardForgeCard);
}

// 导出类供其他模块使用
let exportedClass = customElements.get(CARD_ELEMENT_NAME);
if (!exportedClass) {
  exportedClass = class extends HTMLElement {
    connectedCallback() {
      this.innerHTML = '<div>卡片工坊加载失败：重复注册</div>';
    }
  };
}

export { exportedClass as HaCardForgeCard };